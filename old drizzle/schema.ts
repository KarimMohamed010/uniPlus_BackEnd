import { pgTable, foreignKey, integer, varchar, text, timestamp, primaryKey, check } from "drizzle-orm/pg-core"
...
export const comments = pgTable("comments", {
	id: integer().generatedAlwaysAsIdentity({ name: "comments_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	author: integer(),
	content: text(),
...
export const messages = pgTable("messages", {
	...
	sendAt: timestamp("send_at", { withTimezone: true, mode: 'string' }).defaultNow(),
	content: text(),
...
export const createPost = pgTable("create_post", {
	postId: integer("post_id").primaryKey().notNull(),
...
export const takePlace = pgTable("take_place", {
...
export const rides = pgTable("rides", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "rides_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	toLoc: text("to_loc"),
	fromLoc: text("from_loc"),
	price: integer(),
	seatsAvailable: integer("seats_available"),
	arrivalTime: timestamp("arrival_time", { withTimezone: true, mode: 'string' }).defaultNow(),
...
export const rooms = pgTable("rooms", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "rooms_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
...
export const speakers = pgTable("speakers", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "speakers_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: varchar({ length: 50 }).notNull(),
	bio: text(),
...
export const joinRide = pgTable("join_ride", {
	...
	primaryKey({ columns: [table.studentId, table.rideId], name: "join_ride_pkey"}),
]);
...
export const subscribe = pgTable("subscribe", {
	...
	primaryKey({ columns: [table.userId, table.teamId], name: "subscribe_pkey"}),
]);

export const speak = pgTable("speak", {
	...
	primaryKey({ columns: [table.speakerId, table.eventId], name: "speak_pkey"}),
]);

export const belongTo = pgTable("belong_to", {
	studentId: integer("student_id").notNull(),
	teamId: integer("team_id").notNull(),
	role: text().notNull(),
}, (table) => [
	...
	primaryKey({ columns: [table.studentId, table.teamId], name: "belong_to_pkey"}),
]);
...
export const reports = pgTable("reports", {
	...
	primaryKey({ columns: [table.studentId, table.postId], name: "reports_pkey"}),
]);
...
export const apply = pgTable("apply", {
	...
	primaryKey({ columns: [table.studentId, table.teamId], name: "apply_pkey"}),
]);
...
export const badges = pgTable("badges", {
	...
	expDate: timestamp("exp_date", { withTimezone: true, mode: 'string' }).defaultNow(),
	...
	primaryKey({ columns: [table.studentId, table.teamId], name: "badges_pkey"}),
]);

export const ticketsAndFeedback = pgTable("tickets_and_feedback", {
	eventId: integer("event_id").notNull(),
	studentId: integer("student_id").notNull(),
	certificationUrl: text("certification_url"),
	dateIssued: timestamp("date_issued", { withTimezone: true, mode: 'string' }).defaultNow(),
	...
	primaryKey({ columns: [table.eventId, table.studentId], name: "tickets_and_feedback_pkey"}),
	check("tickets_and_feedback_rating_check", sql`(rating >= 0) AND (rating <= 5)`),
]);
